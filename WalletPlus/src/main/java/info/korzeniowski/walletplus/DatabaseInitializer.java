...
public class DatabaseInitializer {
    ...
    private void fillExampleDatabase() {
        ...
        localCashFlowService.insert(new CashFlow().setAmount(100.0).setCategory(mainHouse).setWallet(personalWallet).setDateTime(date.getTime()).setComment("Food"));

        date.add(Calendar.DATE, -1);
        localCashFlowService.insert(new CashFlow().setAmount(150.0).setCategory(mainHouse).setWallet(personalWallet).setDateTime(date.getTime()).setComment("Cleaning products"));

        date.add(Calendar.HOUR_OF_DAY, -1);
        localCashFlowService.insert(new CashFlow().setAmount(100.0).setWallet(sock).setWallet(personalWallet).setDateTime(date.getTime()).setComment("Transfer to personal wallet"));

        date.add(Calendar.HOUR_OF_DAY, -1);
        localCashFlowService.insert(new CashFlow().setAmount(75.0).setCategory(energy).setWallet(bankAccount).setDateTime(date.getTime()));
        localCashFlowService.insert(new CashFlow().setAmount(100.0).setCategory(water).setWallet(bankAccount).setDateTime(date.getTime()));
        localCashFlowService.insert(new CashFlow().setAmount(50.0).setCategory(gas).setWallet(bankAccount).setDateTime(date.getTime()));

        date.add(Calendar.DATE, -1);
        localCashFlowService.insert(new CashFlow().setAmount(500.0).setWallet(bankAccount).setWallet(personalWallet).setDateTime(date.getTime()));
        localCashFlowService.insert(new CashFlow().setAmount(1000.0).setWallet(bankAccount).setWallet(wardrobe).setComment("Savings").setDateTime(date.getTime()));
        localCashFlowService.insert(new CashFlow().setAmount(3000.0).setCategory(null).setWallet(bankAccount).setComment("Payment").setDateTime(date.getTime()));
    }
}
